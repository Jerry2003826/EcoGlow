...
SELECT
    ...
    so.`grand_total_cents`, so.`tax_cents`,
    COALESCE(SUM(COALESCE(soi.`cost_snapshot_cents`, 0) * soi.`quantity`), 0) AS `cogs_cents`,
    (so.`grand_total_cents` - so.`tax_cents`
      - COALESCE(SUM(COALESCE(soi.`cost_snapshot_cents`, 0) * soi.`quantity`), 0)) AS `estimated_gross_profit_cents`
...
LEFT JOIN (
    SELECT `customer_id`, COUNT(*) AS `order_count`,
           SUM(CASE WHEN `status` <> 'cancelled' THEN `grand_total_cents` ELSE 0 END) AS `lifetime_order_value_cents`,
           MAX(`placed_at`) AS `last_order_at`
      FROM `sales_orders` WHERE `customer_id` IS NOT NULL GROUP BY `customer_id`
...
SELECT
    ...
    COALESCE(SUM(CASE WHEN so.`status` <> 'cancelled' THEN so.`grand_total_cents` ELSE 0 END), 0) AS `gross_sales_cents`,
    COALESCE(SUM(CASE WHEN so.`status` <> 'cancelled' THEN so.`tax_cents` ELSE 0 END), 0) AS `tax_cents`
FROM `sales_orders` so
